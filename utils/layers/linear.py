...
class SplitLinear(nn.Linear):
    def __init__(self, *args, **kwargs):
        ...
        if self.split_mode == 'kels':
            if self.in_channels_order is None:
                if last_layer:
                    active_in_dim = math.ceil(self.weight.size()[1] *
                                              split_rate)
                    mask = np.zeros(
                    ...
                    active_in_dim = math.ceil(self.weight.size()[1] *
                                              split_rate)
                    active_out_dim = math.ceil(self.weight.size()[0] *
                                               split_rate)
                    mask = np.zeros(
                        (self.weight.size()[0], self.weight.size()[1]))
                ...
                for conv in conv_concat:
                    mask[:, start_ch:start_ch +
                         math.ceil(int(conv) * split_rate)] = 1
                    start_ch += int(conv)
    ...
    def extract_slim(self):
        ...
        d_out = sum(self.mask[:, 0] == 1).item()
        d_in = sum(self.mask[0, :] == 1).item()

        if self.in_channels_order is None:
            # assert dst_m.weight.shape == self.weight[:d_out, :d_in].shape
            self.weight.data = self.weight.data[:d_out, :d_in]
            # assert dst_m.bias.data.shape == self.bias.data[:d_out].shape
            self.bias.data = self.bias.data[:d_out]
        else:
            self.weight.data = self.weight[:d_out, self.mask[0, :] == 1]
            self.bias.data = self.bias.data[:d_out]
